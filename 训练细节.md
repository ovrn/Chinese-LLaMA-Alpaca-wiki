...
如果欲了解扩充词表的具体方法，或者使用自己的词表对LLaMA tokenizer进行扩充，我们提供了代码[merge_tokenizers.py](scripts/merge_tokenizers.py)供参考。
该脚本运行方式如下：
```
...
```
其中llama_tokenizer_dir指向存放原版LLaMA tokenizer的目录；chinese_sp_model_file指向用sentencepiece训练的中文词表文件。我们所使用的在中文通用语料上训练好的20K中文词表也一并放出，可以在[scripts/chinese_sp.model](scripts/chinese_sp.model)下载。
